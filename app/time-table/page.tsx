...
function mapPlayerTimeFromServer(rawData: any) {
  ...
  return rawData.reduce((acc: any, item: any) => {
    const existingDay = acc.find((day: any) => day.dateOfWeek === item.dateOfWeek);
...
export default function TimeTable() {
  ...
  // 	{ dateOfWeek: 'SUNDAY', times: [], name: 'Sun' },
  // ]);
  const [playerTimes, setPlayerTimes] = useState<PlayerTime[]>(mapPlayerTimeFromServer(getPlayerTimeTables()));
  ...
  const handleAddTime = () => {
    const index = playerTimes.findIndex((item) => item.dateOfWeek === day.dateOfWeek);
  ...
  return (
    <div className="">
      ...
      <div className="mt-[60px]" style={{ minHeight: 'calc(100vh )' }}>
        <Background>
          <div className="px-3 flex flex-col">
            <h3 className="pt-3 font-medium text-xl">Timetable setup</h3>

            <p className="font-normal text-[14px]">Please set time range that you want to play game</p>
          </div>
              ...
              {playerTimes.map((item, index) => (
                <div className="flex flex-col items-center" key={index}>
                  ...
                  <div
                    ...
                    onClick={() => handleSetTime(item.dateOfWeek, item.name)}
                  >
                    Set time
                  </div>
                </div>
      ...
      <Modal isOpen={isOpen} scrollBehavior={scrollBehavior} onOpenChange={onOpenChange} placement="top">
        <ModalContent>
          {() => (
            <>
              <ModalHeader className="flex flex-col gap-1">{day.name}</ModalHeader>
              <ModalBody>
